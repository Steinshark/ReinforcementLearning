...
class Trainer:
    ...
    def __init__(	self,
        ...
        if not "temp" in os.listdir("C:/Users/Default"):
            os.mkdir("C:/Users/Default/temp")
        if not "models" in os.listdir("C:/Users/Default/temp"):
            os.mkdir("C:/Users/Default/temp/models")
    ...
    def train_concurrent(self,iters=1000,train_every=1024,pool_size=32768,sample_size=128,batch_size=32,epochs=1,transfer_models_every=2,verbose=False,rewards={"die":-3,"eat":5,"step":-.01},max_steps=100,random_pick=True,drop_rate=0):
        ...
        while i < iters and not self.cancelled:
            ...
            #	UPDATE VERBOSE 
            if verbose:
                print(f"\t{Color.blue}[Episode {str(i).rjust(15)}/{int(iters)} -  {(100*i/iters):.2f}% complete\t{(time.time()-t0):.2f}s\te: {e:.2f}\hs: {self.best_score}\t] steps: {(sum(self.all_lived[-100:])/len(self.all_lived[-100:])):.2f} score: {(sum(self.all_scores[-100:])/len(self.all_scores[-100:])):.2f}{Color.end}")
    ...
    def cleanup(self):
        print(f"cleanup run")
        blocked_scores		= reduce_arr(self.all_scores,256)
        blocked_lived 		= reduce_arr(self.all_lived,256)
        if self.gui:
        ...
        if self.gui:
            ...
            self.parent_instance.game_runs[self.parent_instance.settings["run_name"]]['steps'] = blocked_lived
            self.parent_instance.game_runs[self.parent_instance.settings["run_name"]]['scores'] = blocked_scores
        else:
            print(f"Gui false")
    ...
    def train_on_experiences(self,big_set,epochs=1,batch_size=8,early_stopping=True,verbose=False):
        ...
        for epoch_i in range(epochs):
            ...
            #	Telemetry
            if verbose:
                print(f"{Color.blue}\t\tEPOCH: {epoch_i}\tPROGRESS- [",end=f'{Color.tan}')
            ...
            #	Telemetry
            if verbose :
                print(f"{Color.blue}]\ttime: {(time.time()-t0):.2f}s\tt_gpu:{(t_gpu):.2f}\tloss: {(total_loss/num_batches):.6f}{Color.end}")
        if verbose:
            print("\n\n")
...
if __name__ == "__main__":
    ...
    t =                 Trainer( 10,
                                10,
                                model_class     = networks.SnakeAdaptNet,
                                input_dim       = (3,10,10),
                                loss_fn         = torch.nn.MSELoss,
                                optimizer_fn    = torch.optim.Adam,
                                activation      = torch.nn.functional.relu,
